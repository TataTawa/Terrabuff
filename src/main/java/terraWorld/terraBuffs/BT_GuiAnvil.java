...
public class BT_GuiAnvil extends GuiCommon{
	    ...
	    if(this.genericTile != null)
	    {
	    	...
	    	if(anvil.getStackInSlot(1) != null)
	    	{
		    	...
		    	}else
		    	{
		    		...
		    		if(reforgable)
		    			this.fontRendererObj.drawStringWithShadow(reqExp+"LV "+reqExp/3+" Gold", k+70, l1+26, k1);
		    		else
		    			this.fontRendererObj.drawStringWithShadow(reqExp+"LV "+reqExp/3+" Gold", k+70, l1+26, 0xff4444);
		    	}
    ...
    {
    	if(!BT_Utils.itemHasEffect(stk))return 12;
    	...
    	if(tag.hasKey("BT_Buffs"))
    	{
			...
			double sum=0;
			for(DummyData dummyData : data)
			{
				if(!dummyData.fieldName.equals(BT_Mod.KNOCK_BACK) && !dummyData.fieldName.equals("magicFind"))sum += Double.valueOf(dummyData.fieldValue);
			}
			if(sum<-0.75) return 3;
    		return (int)(12 * (1+sum));
    	}
    	return 0;
    }
