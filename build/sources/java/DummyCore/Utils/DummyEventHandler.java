...
public class DummyEventHandler {
	...
	static {
		try {
			ReflectionHelper.findField(World.class, "worldAccesses");
			obfuscated=false;
		} catch (Exception e) {
			obfuscated=true;
		}
				...
				if(modData.fieldName.equalsIgnoreCase("mod") && modData.fieldValue.equalsIgnoreCase("dummycore.particle"))
				{
					...
					for(int i=0;i<(packetData.length-2)/9;i++) {
						...
						if (r == 0 && g == 0 && b == 0) {
							...
							try {
								List worldAccesses = ObfuscationReflectionHelper.getPrivateValue(World.class,event.recievedEntity.worldObj,obfuscated?"u":"worldAccesses");
								//List worldAccesses = (List) ReflectionHelper.findField(World.class, obfuscated?"u":"worldAccesses").get(event.recievedEntity.worldObj);
